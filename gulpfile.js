...
gulp.task("concat-html", () => {
	let files = [
		src + "template/template-start.html"
	];
	...
	files = files.concat([
		dist + "index.html",
		src + "template/template-end.html"
	]);
		...
		.pipe(cheerio({
			run: function ($, file) {
				...
				$('[fill^="url("]').each(function (index, object) {
					let newUrl = $(object).attr('fill').replace(/url\(#(.+)\)/g, 'url(#' + filename + '_$1)');
					$(object).attr('clip-path', newUrl);
				});
...
gulp.task("build", gulp.series("svg-min", "copy-template", "compile-html", "concat-html", "copy-images", "build-js", "build-css", "copy-js-libs"));
